...
void onInit(CBlob@ this)
{
	...
	//Sound
	settings.FIRE_SOUND = "SilencedAK47_Shoot.ogg"; //Sound when shooting
	//settings.RELOAD_SOUND = "SMGReload.ogg"; //Sound when reloading

	//Offset
	settings.MUZZLE_OFFSET = Vec2f(-23, -0.5); //Where the muzzle flash appears
	...
	if (laser !is null)
	{
		...
		laser.SetOffset(Vec2f(-15.0f, 0.0f));
	}
}
...
/*
		...
		if (flip)
		{	
			length = (hitPos - startPos + sus).Length();
		}
		else
		{
			length = (hitPos - startPos - sus).Length();
...
*/
		...
		if (laser !is null)
		{
			...
			if (holder.isMyPlayer()) laser.SetVisible(false);
		}
	}
	else
	{
